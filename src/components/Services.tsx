                ...
                <article className="group grid overflow-hidden rounded-[1.75rem] border border-beige/70 bg-cream-soft shadow-soft transition-shadow hover:shadow-lift lg:grid-cols-2">
                  ...
                  <div className={`flex flex-col p-7 md:p-10 ${idx % 2 === 1 ? "lg:order-1" : ""}`}>
                    <div className="flex items-center justify-between">
                      ...
                      <span aria-hidden className="text-5xl font-bold text-beige">{service.number}</span>
                    </div>
                    <h3 className="mt-5 text-2xl font-bold text-brand md:text-[1.7rem]">{service.title}</h3>
                    <p className="mt-3 text-[15px] leading-8 text-brand-ink/70">{service.description}</p>
                    {service.bullets && (
                      <ul className="mt-5 grid gap-2.5 sm:grid-cols-2">
                    ...
                    <div className="mt-auto flex flex-wrap items-center gap-3 pt-6">
                      <a href="#contact" className="rounded-full bg-brand px-6 py-2.5 text-sm font-bold text-cream shadow-card transition-all hover:-translate-y-0.5 hover:bg-brand-deep">اطلب الخدمة</a>
                      <a href={PHONE_TEL} className="group flex items-center gap-2 text-sm font-bold text-brand hover:text-brand-deep">
                        <PhoneIcon className="h-4 w-4 text-accent transition-transform group-hover:-rotate-12" />
                        <span dir="ltr" className="tracking-wide">{PHONE_DISPLAY}</span>
                      </a>
                    </div>
